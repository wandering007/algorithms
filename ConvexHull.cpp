...
void GrahamScan_2D()//Graham Scan只能用于2D情况，时间复杂度O(nlogn)
{
    ...
    int pos=0;
    for ( int i = 1; i < n ; i++ )
        if((point[i].y < point.y)||(point[i].y == point.y && point[i].x < point.x))
            {
                p = point[i];
                pos = i;
            }
    ...
    for ( int i = 2; i <= n ; ++ i )
    {
        while (top>=1&&cross(S[top - 1], S[top], point[i]) < 0 )//cross返回值为0也可以包括进去，视情况而定
            top -- ;                          //出栈
...
{
    sort(point, point + MAX_SIZE + 1, ccmp);//sorted by increasing x and y-coordinates
   // Build lower hull
    int top = -1;
    for (int i = 0; i < n; i++) 
    {
        while (top >= 1 && cross(S[top-1], S[top], point[i]) <= 0)//加等于号很重要!保证凸包顶点的正确性
            top--;
        S[++top] = point[i];
    }
    // Build upper hull
    ...
    for (int i = n-2; i >= 0; i--) 
    {
        while (top >= t && cross(S[top-1], S[top], point[i]) <= 0)//加等于号很重要!保证凸包顶点的正确性
            top--;
        S[++top] = point[i];
    }
    return top;
