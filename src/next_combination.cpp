...
#include <algorithm>
/*-------------------------- Implementation I -------------------------------*/
//v - contains all elements
//sel - contains selected elements
#include <vector>
int n, k; //n - number of total elements, k - number of selected elements

/*put in main function
  std::vector< int > isRetain(n, 0);
  std::fill(isRetain.begin(), isRetain.begin() + k, 1);
*/
// a quite ugly implementation

bool next_combination(std::vector< int > &v)
{ 
    int t = 0;
	if(next_permutation(isRetain.begin(), isRetain.end()))
 	{
		for (int i = 0; i < n; ++i)
 		{
           if (1 == isRetain[i])
